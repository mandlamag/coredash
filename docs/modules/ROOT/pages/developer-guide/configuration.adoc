...
....
{
    ...
    "customHeader": "",
    "ALLOW_QUERIES_WITHOUT_LOGIN": false,
}
....
...
|ALLOW_QUERIES_WITHOUT_LOGIN |boolean |false |When enabled, allows users to run queries without providing authentication credentials. This adds a "Skip Login" button to the connection modal that connects to a default GraphQL API endpoint. This is particularly useful for development and testing scenarios.
